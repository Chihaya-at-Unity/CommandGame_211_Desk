...
public class PlayerAttack3 : MonoBehaviour
{
    ...
    {
        { 3, 3, 6, 6, 6, 1},//配列属性数が　6番目（固定）に保存されているため、注意！
        { 3, 3, 4, 4, 6, 2},//配列属性数が　6番目（固定）に保存されているため、注意！
        { 0, 0, 3, 4, 5, 4},//配列属性数が　6番目（固定）に保存されているため、注意！
    };
    ...
    public void Get_Set_Commnad()
    {
        ...
        if (AA > -1)
        {
            ...
            {
                case 1:
                    Debug.Log("player3赤は" + Type_Count[AA]);
                    break;
                case 2:
                    Debug.Log("player3青は" + Type_Count[AA]);
                    break;
                case 3:
                    Debug.Log("player3黄は" + Type_Count[AA]);
                    break;
                case 4:
                    Debug.Log("player3紫は" + Type_Count[AA]);
                    break;
                case 5:
                    Debug.Log("player3茶は" + Type_Count[AA]);
                    break;
                case 6:
                    Debug.Log("player3緑は" + Type_Count[AA]);
                    break;
            }*/
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (level[i] == Com_Normal[i, 5])
            {
                ...
                if (i == 0)
                {
                    ...
                    while (kaisuu > 0)
                    {
                        Debug.Log("player3通常攻撃" + kaisuu + "回できる");
                        kaisuu--;
                    }
                }

                if (i == 1) Debug.Log("player3特殊スキル　１　発動可能！");//毎時１回のみ
                if (i == 2) Debug.Log("player3特殊スキル　２　発動可能！！");//毎時１回のみ
            }
        }
    }
